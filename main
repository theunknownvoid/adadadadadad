...
local AzureWindow = UiLib:CreateWindow("Azure Modded | modified by senn", Vector2.new(550, 350), Enum.KeyCode.Insert)

-- Tabs

local MainTab = AzureWindow:CreateTab("Main")
local MiscTab = AzureWindow:CreateTab("Misc")
...
AntiLockSec:AddSlider("Reverse Amount", 0.5, 3.5, 100, 2, function(Value)
...
AntiLockSec:AddSlider("Desync Angles", -50, 0.1, 50, 2, function(Value)
...
spawn(function()
	...
	StatsPicture.Name = "StatsPicture"
	...
	StatsHealthBackground.Name = "StatsHealthBackground"
	...
	StatsHealthBar.Name = "StatsHealthBar"
...
RunService.Heartbeat:Connect(function()
	...
	if AntiLock.Enabled then
		...
		elseif AntiLock.Mode == "Prediction Reverser" then
			realreverse = AntiLock.PredReverseAmt + (math.random(1,AntiLock.PredReverseAmt) / AntiLock.PredReverseAmt *  math.random(1,7))
			RootPart.CFrame = Cframe - LocalPlr.Character.Humanoid.MoveDirection * (realreverse / 10)
...
RunService.Stepped:Connect(function()
    if game.Players.LocalPlayer.PlayerGui:FindFirstChild('JumpCooldown') then
	   game.Players.LocalPlayer.PlayerGui.JumpCooldown.Enabled = not Utilities.NoJumpCooldown
    end
end)
	...
	if CameraAimbot.Enabled and CamBindEnabled then
		if CameraAimbot.AimMethod == "Camera" then
			if CameraAimbot.Resolver then
				...
				if CameraAimbot.Smoothing then
					Workspace.CurrentCamera.CFrame = Workspace.CurrentCamera.CFrame:Lerp(CFrame.new(Workspace.CurrentCamera.CFrame.p, CamlockPlr.Character[CameraAimbot.RealHitPart].Position + Vector3.new(0, CameraAimbot.RealJumpOffset, 0) + (CamlockPlr.Character[CameraAimbot.RealHitPart].Velocity * CameraAimbot.RealPrediction)), CameraAimbot.Smoothness, Enum.EasingStyle.Circular, Enum.EasingDirection.InOut)
				else
...
__index = hookmetamethod(game, "__index", function(self, key)
    ...
    if self == game.Players.LocalPlayer:GetMouse() and (tostring(key) == "Hit" or tostring(key) == "Target") then
	    if TargetAimbot.Enabled and TargBindEnabled then
	    	...
	    	if TargetAimbot.Resolver then
	    	    ...
	    	    Arguments[4] = TargetPlr.Character[TargetAimbot.RealHitPart]
			else
...
__index = hookmetamethod(game, "__index", function(self, key)
    if self == Mouse and (tostring(key) == "Hit" or tostring(key) == "Target") then
		if TargetAimbot.Enabled and TargBindEnabled then
		    local Arguments = {}
            if TargetAimbot.Resolver then
		    	Arguments[3] = TargetPlr.Character[TargetAimbot.RealHitPart].CFrame + Vector3.new(0, TargetAimbot.RealJumpOffset, 0) + (TargetPlr.Character.Humanoid.MoveDirection * TargetAimbot.RealPrediction * TargetPlr.Character.Humanoid.WalkSpeed)
		    	Arguments[4] = TargetPlr.Character[TargetAimbot.RealHitPart]
		    else
		    	Arguments[3] = (TargetPlr.Character[TargetAimbot.RealHitPart].CFrame + (TargetPlr.Character[TargetAimbot.RealHitPart].Velocity * TargetAimbot.RealPrediction)) + Vector3.new(0, TargetAimbot.RealJumpOffset, 0) 
		    	Arguments[4] = TargetPlr.Character[TargetAimbot.RealHitPart]
		    end
			print(Arguments[3])
			print(Arguments[4])
